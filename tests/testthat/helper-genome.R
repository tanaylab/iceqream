...
# Returns list(base = <trajectory 1->4>, base2 = <trajectory 1->2>), both
# regressed on a small subset of the cached gastrulation data with
# additional_features = NULL. Calls skip_*() when prerequisites are missing.
genome_traj_models <- local({
    ...
    function() {
        ...
        needed <- file.path(cdir, c("peak_intervals.rds", "atac_scores.rds", "motif_energies.rds"))
