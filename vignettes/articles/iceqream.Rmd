...
library(misha)
library(misha.ext)
options(timeout = 2 * 60 * 60) # 2 hours for loading large files
```
...
# motif_energies <- compute_motif_energies(peak_intervals, db, normalization_intervals = normalization_intervals)
```

However, we will load the precomputed motif energies for this example.
...
Note that the next step requires ~23GB of RAM.
...
traj_model <- iq_regression(
    ...
    frac_train = 0.8,
    max_motif_num = 30,
    n_prego_motifs = 0 # increase this in order to infer de-novo motifs, e.g. 5
)
