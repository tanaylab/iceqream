...
motif_energies <- compute_motif_energies(peak_intervals, db = motif_db, normalization_intervals = normalization_intervals)
```
...
motif_energies <- compute_motif_energies(peak_intervals, db = motif_db_scenic_clusters, normalization_intervals = normalization_intervals)
```
...
By default, `iq_regression()` automatically renames the motif models to more informative names by matching them against the HOMER database (controlled by the `rename_motifs` argument, default `TRUE`). The mapping that was applied is stored on the model:

```{r names_map}
head(traj_model@params$names_map)
```

**Important caveat:** the assigned name should be treated as a *label*, not a TF identification. The mapping picks the single best-correlated PSSM in the reference database, but (1) many TFs share a motif with other family members, so the chosen name is one of several plausible TFs that could bind that sequence, and (2) the best correlation can still be modest — a low-similarity match will silently get a name. Inspect the PSSM logo and the correlation in `match_traj_model_motif_names()` output before claiming a specific TF is responsible.

If you set `rename_motifs = FALSE` and want to apply the mapping later — or supply a custom one — use `match_traj_model_motif_names()` to derive the map and `rename_motif_models()` to apply it:

```{r rename_motif_models, eval = FALSE}
